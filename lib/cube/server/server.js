...
module.exports = function (options) {
  ...
  udp.on("message", function (message, request) {
    endpoints.udp[0].dispatch(JSON.parse(message));
  });
  ...
  server.start = function () {
    ...
    if (cluster.isMaster) {
      ...
      for (var i = 0; i < numCPUs; i++) {
        ...
        util.log('worker started with pid ' + worker.pid);
      }
      ...
      //There seems to be a bug in node because I get an EADDRINUSE
      //error when I attempt to bind this port in the worker.
      if (options["udp-port"]) {
